...
import { BellIcon, ChevronDownIcon, ChevronRightIcon } from './icons/Icons';
...
const Notifications: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 non-printable">
            {isOpen && (
                <Card className="absolute bottom-16 right-0 w-80 mb-2 shadow-2xl">
                    ...
                    <div className="space-y-3 max-h-80 overflow-y-auto">
                        {totalNotifications > 0 ? (
                            ...
                            <div className="text-sm text-secondary-text p-2 text-center">
            ...
            <button 
                ...
                className="bg-accent text-white p-3 rounded-full shadow-lg hover:bg-accent-hover transition-transform duration-200 hover:scale-110 relative"
                aria-label="Показати сповіщення"
                aria-expanded={isOpen}
            >
                <BellIcon />
                {totalNotifications > 0 && (
                    <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full border-2 border-card animate-[pulse-red-dot_2s_infinite]">
                        {totalNotifications}
                    </span>
                )}
            </button>
